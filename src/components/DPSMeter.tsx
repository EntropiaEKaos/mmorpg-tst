    ...
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/68 p-4 backdrop-blur-md" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="moria-panel moria-fade-up w-full max-w-2xl rounded-3xl border border-rose-300/20 p-4 sm:p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <div className="moria-eyebrow text-rose-300">Combat analytics</div>
            <h2 className="moria-title mt-1 text-2xl font-black">📊 DPS Meter</h2>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => { dpsMeter.clear(); setTick((t) => t + 1); }} className="moria-button rounded-lg px-3 py-1.5 text-[10px] font-bold text-rose-200">↻ RESET</button>
            <button onClick={onClose} className="moria-button flex h-8 w-8 items-center justify-center rounded-lg text-sm text-slate-400" aria-label="Close DPS meter">✕</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          ...
          <StatBox label="Duration" value={`${stats.duration}s`} color="#8fc8ff" icon="⏱" />
          <StatBox label="Total Damage" value={stats.totalDamage} color="#ff9aa5" icon="💥" />
          <StatBox label="Total Healing" value={stats.totalHealing} color="#8de8bd" icon="✨" />
          <StatBox label="Crit Rate" value={`${stats.critRate}%`} color="#e5c477" icon="🎯" />
          <StatBox label="Hits" value={stats.hits} color="#cbd5e1" icon="👊" />
          <StatBox label="Crits" value={stats.crits} color="#f4d47c" icon="✦" />
          <StatBox label="Max Hit" value={stats.maxHit} color="#ff6677" icon="☠" />
        </div>

        {recent.length > 0 ? (
          <div className="mt-4">
            <div className="mb-2 flex items-center justify-between">
              <div className="moria-eyebrow text-[8px]">RECENT COMBAT</div>
              <div className="text-[9px] text-slate-600">latest {recent.length}</div>
            </div>
              ...
              {[...recent].reverse().map((record, index) => {
                ...
                return (
                  <div key={`${record.timestamp}-${record.target}-${index}`} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-[10px] hover:bg-white/[0.025]">
                    ...
                    <span className="min-w-0 flex-1 truncate text-slate-400">{record.target}</span>
                    <span className="shrink-0 font-black" style={{ color }}>{heal ? '+' : '-'}{record.amount.toLocaleString()}</span>
                    <span className="w-14 shrink-0 text-right text-[8px] uppercase text-slate-600">{record.type}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="moria-card mt-4 rounded-2xl p-7 text-center">
            ...
            <div className="moria-eyebrow mt-3">Awaiting combat</div>
            <div className="mt-2 text-xs text-slate-500">Damage and healing will appear automatically from your first combat event.</div>
          </div>
        )}
      </div>
    </div>
