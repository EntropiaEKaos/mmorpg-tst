        ...
        <div className="moria-scrollbar flex-1 overflow-y-auto p-4 text-sm">
          ...
          {tab === 'guild' && <div className="grid gap-4 lg:grid-cols-2">
            <div className={card}>
              ...
              {!guild ? <div className="mt-3 space-y-2"><input value={guildName} onChange={e => setGuildName(e.target.value)} className={input} maxLength={32} placeholder="Guild name"/><button disabled={player.level < 10 || player.gold < 1000 || guildName.trim().length < 3} onClick={() => act('guild_create', { name: guildName })} className={`${button} w-full`}>Create guild · 1000g · Lv10+</button></div> : <>
                <h3 className="mt-3 text-xl font-black text-amber-100">🛡 {guild.name}</h3><div className="text-xs text-slate-400">{tr('Role:')} {tr(guild.selfRole)}</div><p className="mt-2 rounded bg-black/30 p-2 text-xs text-slate-300">{guild.motd || 'No guild message.'}</p>
            ...
            <div className={card}><div className="moria-eyebrow text-emerald-300">MEMBERS & RECRUITING</div>{guild && <div className="mt-3 space-y-2">{guild.members?.map((member: any) => <div key={member.key} className="flex items-center justify-between rounded-lg bg-slate-900/60 p-2"><span>{member.name} · {tr(member.role)} {member.online ? '🟢' : '⚫'}</span>{guild.selfRole === 'leader' && member.key !== player.name.toLowerCase() && <div className="flex gap-1"><button onClick={() => act('guild_role', { targetKey: member.key, role: member.role === 'officer' ? 'member' : 'officer' })} className={button}>{member.role === 'officer' ? 'Demote' : 'Officer'}</button><button onClick={() => act('guild_kick', { targetKey: member.key })} className={button}>Kick</button></div>}</div>)}</div>}{guild && ['leader','officer'].includes(guild.selfRole) && <div className="mt-4 space-y-2">{nearby.filter((p: any) => !guild.members?.some((m: any) => m.key === p.name.toLowerCase())).map((p: any) => <div key={p.id} className="flex justify-between"><span>{p.name}</span><button onClick={() => act('guild_invite', { targetId: p.id })} className={button}>Invite</button></div>)}</div>}</div>
          </div>}
          ...
          {tab === 'trade' && <div className="grid gap-4 lg:grid-cols-2">
            <div className={card}>
              ...
              {!trade ? <div className="mt-3 space-y-2">{nearby.map((p: any) => <div key={p.id} className="flex items-center justify-between rounded-lg bg-slate-900/60 p-2"><span>{p.name} · {tr('Lv')} {p.level}</span><button onClick={() => act('trade_request', { targetId: p.id })} className={button}>Request trade</button></div>)}</div> : <>
                <div className="mt-3 space-y-3">{trade.players?.map((entry: any) => <div key={entry.key} className="rounded-xl border border-slate-700/60 p-2"><div className="flex justify-between"><b>{entry.self ? tr('You') : entry.name}</b><span className={entry.confirmed ? 'text-emerald-300' : 'text-slate-500'}>{entry.confirmed ? '✓ Confirmed' : 'Not confirmed'}</span></div><div className="text-amber-300">🪙 {entry.gold}g</div><div className="mt-1 flex flex-wrap gap-1">{entry.items?.map((item: any) => <span key={item.id} className="rounded bg-slate-900 px-2 py-1 text-xs">{item.icon} {tr(item.name)} ×{item.quantity}</span>)}</div></div>)}</div>
