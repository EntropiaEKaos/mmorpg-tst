  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/65 p-5 backdrop-blur-md" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="moria-panel moria-fade-up relative w-full max-w-xl overflow-hidden rounded-3xl border p-5 sm:p-6" style={{ borderColor: `${npc.color}66`, boxShadow: `0 30px 90px rgba(0,0,0,.55), 0 0 42px ${npc.color}18` }}>
        ...
        <button onClick={onClose} className="moria-button absolute right-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-lg text-sm text-slate-400" aria-label="Close dialogue">✕</button>
        ...
        <div className="relative flex items-start gap-4 pr-10">
          ...
          <div className="min-w-0 flex-1">
            <div className="moria-eyebrow" style={{ color: npc.color }}>{npc.role || 'Wanderer'}</div>
            <h2 className="moria-title mt-1 truncate text-2xl font-black">{npc.name}</h2>
            <div className="mt-3 rounded-2xl border border-white/[0.06] bg-black/25 p-3 text-sm italic leading-6 text-slate-300">
              <span className="mr-1 text-xl leading-none" style={{ color: npc.color }}>“</span>{dialogue.text}<span className="ml-1 text-xl leading-none" style={{ color: npc.color }}>”</span>
            </div>
          </div>
        </div>
        ...
        {hasCompletedQuest && (() => {
          ...
          return quest ? (
            <div className="moria-card relative mt-4 rounded-2xl border-amber-200/25 p-3">
              ...
              <div className="text-sm font-bold text-slate-100">{quest.name}</div>
              <div className="mt-1 text-[10px] text-slate-400">Reward · {quest.rewards.xp} XP · {quest.rewards.gold} gold</div>
            </div>
          ) : null;
        })()}
              ...
              <div className="space-y-1">
                {aq?.objectives.map((o, i) => (
                  <div key={i} className="flex items-center justify-between text-[11px] text-slate-400">
                    <span>{o.current >= o.count ? '✓' : '○'} {o.targetName}</span>
        ...
        {npc.shop && npc.shop.length > 0 && (
          <div className="relative mt-5 border-t border-white/[0.06] pt-4">
            <div className="moria-eyebrow mb-2">Merchant stock</div>
