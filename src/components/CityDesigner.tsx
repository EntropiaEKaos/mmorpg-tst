...
export default function CityDesigner({ onApplied }: Props) {
  ...
  const addAt = (at: { x: number; y: number }) => {
    ...
    if (tool === 'landmark') {
      ...
      const entry: CityLandmark = { id: `${draft.id}_${slug(landmarkName)}_${stamp}`, name: landmarkName.trim().slice(0, 60) || 'Landmark', kind: landmarkKind, icon: iconFor(landmarkKind), x: clamp(at.x, 1, mapWidth - w - 1), y: clamp(at.y, 1, mapHeight - h - 1), w, h };
  ...
  const duplicateSelected = () => {
    ...
    setDraft((current) => {
      if (selection.type === 'landmark') { const source = current.landmarks.find((x) => x.id === selection.id); if (!source || current.landmarks.length >= landmarkLimit) return current; const copy = { ...source, id: `${source.id}_copy_${stamp}`, name: `${source.name} Copy`, x: clamp(source.x + 1, 1, mapWidth - source.w - 1), y: clamp(source.y + 1, 1, mapHeight - source.h - 1) }; setSelection({ type: 'landmark', id: copy.id }); return { ...current, landmarks: [...current.landmarks, copy] }; }
      if (selection.type === 'district') { const source = current.districts.find((x) => x.id === selection.id); if (!source || current.districts.length >= districtLimit) return current; const copy = { ...source, id: `${source.id}_copy_${stamp}`, name: `${source.name} Copy`, x: clamp(source.x + 1, 1, mapWidth - 2), y: clamp(source.y + 1, 1, mapHeight - 2) }; setSelection({ type: 'district', id: copy.id }); return { ...current, districts: [...current.districts, copy] }; }
  ...
  return <div className="grid gap-3 xl:grid-cols-[330px_minmax(420px,1fr)_350px]">
    <section className="space-y-3 rounded border border-cyan-400/25 bg-black/40 p-3">
      ...
      <label className="block text-[9px] font-bold uppercase tracking-wider text-purple-200/60">Map<select data-city-designer-map-select="true" value={mapId} onChange={(e) => chooseMap(e.target.value)} className="mt-1 w-full rounded border border-purple-500/35 bg-black/65 px-2 py-2 text-xs text-purple-100">{ids.map((id) => <option key={id} value={id}>{MAPS[id].name}</option>)}</select></label>
