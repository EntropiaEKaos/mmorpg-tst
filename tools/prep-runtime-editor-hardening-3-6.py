...
new_render = '''      <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-2 py-1 rounded bg-black/60 border border-purple-700/40 text-purple-100">\n        {options.map((o) => <option key={o} value={o}>{optionLabels?.[o] || o}</option>)}\n      </select>\n'''
if old_render in s:
    s = s.replace(old_render, new_render, 1)
elif new_render not in s:
    raise SystemExit('SelectField render pattern missing')

old_usage = "          {tab === 'items' && <ItemEditor player={player} />}\n"
new_usage = "          {tab === 'items' && <ItemEditor />}\n"
if old_usage in s:
    s = s.replace(old_usage, new_usage, 1)
elif new_usage not in s:
    raise SystemExit('ItemEditor usage pattern missing')

old_item_sig = "function ItemEditor({ player }: { player: Player }) {\n"
new_item_sig = "function ItemEditor() {\n"
if old_item_sig in s:
    s = s.replace(old_item_sig, new_item_sig, 1)
elif new_item_sig not in s:
    raise SystemExit('ItemEditor signature pattern missing')

p.write_text(s)
print('3.6 editor compatibility prepared')
