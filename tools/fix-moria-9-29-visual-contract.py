...
p=Path('server/test/reference-visual-9-7.test.mjs')
t=p.read_text()
t=t.replace("assert.match(avatar, /PIXEL_SPRITE_SCALE = 1\\.30/);","assert.match(avatar, /PIXEL_SPRITE_SCALE = 1\\.42/); // 9.29 intentionally supersedes the 9.7 scale contract")
p.write_text(t)
Path('server/test/visual-rebirth-9-27-29.test.mjs').write_text(r'''import test from 'node:test';
...
''')
print('9.29 visual contract updated')
